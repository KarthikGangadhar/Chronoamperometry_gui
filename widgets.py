...
class cl_canvas_frame :
  ...
  def key_pressed_callback( self, event ) :
    msg = '{0} ({1})'.format(event.char,ord( event.char )) if len( event.char ) > 0 else '<non-printing char>'

    self.master.statusBar_frame.set(
      '{0} pressed at ({1},{2})'.format(msg,event.x,event.y ) )
  ...
  def left_mouse_click_callback( self, event ) :
    self.master.statusBar_frame.set('LMB down. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y
    self.canvas.focus_set()

  def left_mouse_release_callback( self, event ) :
    self.master.statusBar_frame.set( 'LMB released. ({0}, {1})'.format(event.x,event.y))
    self.x = None
    self.y = None

  def left_mouse_down_motion_callback( self, event ) :
    self.master.statusBar_frame.set( 'LMB dragged. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y

  def middle_mouse_click_callback( self, event ) :
    self.master.statusBar_frame.set( 'MMB down. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y
    self.canvas.focus_set()

  def middle_mouse_release_callback( self, event ) :
    self.master.statusBar_frame.set( 'MMB released. ({0}, {1})'.format(event.x,event.y) )
    self.x = None
    self.y = None

  def middle_mouse_down_motion_callback( self, event ) :
    self.master.statusBar_frame.set( 'MMB dragged. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y

  def right_mouse_click_callback( self, event ) :
    self.master.statusBar_frame.set( 'RMB down. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y

  def right_mouse_release_callback( self, event ) :
    self.master.statusBar_frame.set( 'RMB released. ({0}, {1})'.format(event.x,event.y) )
    self.x = None
    self.y = None

  def right_mouse_down_motion_callback( self, event ) :
    self.master.statusBar_frame.set( 'RMB dragged. ({0}, {1})'.format(event.x,event.y) )
    self.x = event.x
    self.y = event.y
  ...
  def canvas_resized_callback( self, event ) :
    ...
    self.master.statusBar_frame.set(
      'Canvas width, height ({0}, ' + '{1})'.format(self.canvas.cget( "width" ), self.canvas.cget( "height" )) )
...
class cl_menu :
  ...
  def menu_callback( self, which = None ) :
    ...
    self.master.statusBar_frame.set( '{0} callback'.format(item) )
